...
export const BrowserHelpers = {
  ...
  getRequestParamsList: (filters, config) => {
    ...
    if ('khachHang' in filters) {
      params.khachHang = filters.khachHang
        ? filters.khachHang.map(item => item.value).join(',')
        : ''
    }
    if ('loai' in filters) {
      params.loai = filters.loai ? filters.loai.value : ''
    }
    if ('trangthai' in filters) {
      params.trangthai = filters.trangthai ? filters.trangthai.value : ''
    }
